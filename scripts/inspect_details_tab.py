"""List all fields on the 'Details' tab of the Contact for Utilities (Interactive) form."""
...
for form in forms:
    ...
    for tab in root.findall(".//tab"):
        ...
        if "DETAILS" not in tab_name.upper() and "details" not in label_text.lower():
            continue

        print(f"\nTab: {label_text!r} (name={tab_name})")
        print("-" * 60)
            ...
            for row in section.findall(".//row"):
                for cell in row.findall("cell"):
                    ...
                    if datafieldname:
                        ...
                        print(f"    Control: {ctrl_id:<43} ClassID: {classid[:20]}")
